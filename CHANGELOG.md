...
Resolved the "Proxy account not found" and "Bank account not found" errors and confirmed the full on-ramp order flow working end-to-end in the Etherfuse sandbox.

### Root cause of previous failures

The `bankAccountId` used in earlier order attempts (`6bf8de9d-...`) was a pre-existing sandbox account linked to the org ID as `customerId`, not to the app's auto-generated `customerId` (`239f21ff-...`). Etherfuse requires that the `customerId` and `bankAccountId` in every order request exactly match the IDs that were passed when generating the onboarding URL. Using mismatched IDs causes the "Proxy account not found" error even after T&C acceptance.

### Fix

1. Generated a fresh UUID for `bankAccountId` (`a1f25466-...`) and stored it in `etherfuse.json`
2. Called `POST /ramp/onboarding-url` with the matching `customerId` + `bankAccountId`
3. Opened the presigned URL, completed the full Plaid flow using the **Personal** tab with the pre-filled bank information
4. Updated `etherfuse.json` `bank_account_id` to the new UUID — both IDs now match the onboarding URL
...
| `POST /ramp/order` | ✅ 200 — order created, CLABE returned |

Order response:
```json
{
  "onramp": {
    "orderId": "c12b917e-fe1a-42c3-868c-a110dff27393",
    "depositClabe": "646180615200000582",
    "depositAmount": "100"
  }
}
```

The CLABE (`646180615200000582`) is the Mexican bank account number for the MXN transfer. In production, a user would send 100 MXN to this CLABE via their bank; Etherfuse then deposits the CETES to the wallet's Stellar address automatically.

### Key insight: ID consistency

The `customerId` and `bankAccountId` passed to every quote/order call must be the same UUIDs that were passed to the onboarding URL that the user completed. The `customerId` is auto-generated by the app on first run. The `bankAccountId` must be a UUID you generate yourself, stored in `etherfuse.json` before clicking "Generate onboarding link" — Etherfuse creates and links the bank account to that UUID when the user completes Plaid.
